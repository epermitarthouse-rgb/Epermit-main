...
|------------|--------|----------|---------|------|---------------|----------------|-------------|------|
| ACC-001 | Access | Activate user | Unban + restore login | platform_admin | User deactivated | Auth `ban_duration:none` + `profiles.access_status=active` via `POST .../activate` | `user.activated` | medium |
| ACC-002 | Access | Deactivate user | Block all access | platform_admin | Not last admin; not self if last | Auth ban + `access_status=deactivated` + all credential grants → `none` via `POST .../deactivate` | `user.deactivated` | high |
...
| ACC-007 | Access | Set feature permission | none / read / write | platform_admin | Project membership if project-scoped | Upsert `user_feature_permissions` | `feature_permission.changed` | high |
...
| ACC-009 | Access | Set credential grant | none / use / manage | platform_admin | Credential exists; revocable from creator | Upsert `user_portal_credential_grants` | `credential_grant.changed` | high |
| ACC-010 | Access | Revoke credential grant (incl. creator) | Set grant to none | platform_admin | No owner bypass | Upsert grant_level none | `credential_grant.revoked` | critical |
