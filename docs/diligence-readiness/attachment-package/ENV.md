...
| `QB_ENV` | Railway scraper (QuickBooks) | 3. Optional feature configuration | Optional / context-dependent | — | Yes | Yes | Inferred (status endpoint reported production env); other QB_* not verified individually | No | Requires manual confirmation | Requires manual confirmation | — |
...
| `QB_TOKEN_ENCRYPTION_KEY` | Railway scraper (QuickBooks) | 1. Production-required secrets | Required (production) | — | Yes | Yes | Requires manual confirmation | Yes | Client confirmed (category); per-variable reconciliation required | Requires manual confirmation | — |
