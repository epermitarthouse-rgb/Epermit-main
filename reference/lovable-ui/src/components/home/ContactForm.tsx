...
const schema = z.object({
  ...
  message: z.string().trim().min(10, "Tell us a bit more (10+ characters)").max(1000, "Keep it under 1000 characters"),
});
...
export const ContactForm = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("Message received", {
      description: `Thanks ${parsed.data.name.split(" ")[0]} — Ian will reach out within one business day.`,
    });
    setSubmitted({
      name: parsed.data.name,
  ...
  const steps = [
    {
      ...
      body: `We logged your ${projectType.toLowerCase()} inquiry and routed it to Ian. Watch ${email} for a reply within one business day.`,
    },
    {
      icon: CalendarClock,
      title: "Scoping call (optional)",
      body: "If your project needs eyes-on right away, grab a 20-minute working session directly with Ian — no obligation.",
    },
    {
      icon: FileText,
