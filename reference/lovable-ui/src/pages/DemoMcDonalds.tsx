...
import mcdLogo from "@/assets/mcdonalds-logo.png.asset.json";
...
const tourSteps: TourStep[] = [
  ...
  },
  {
    ...
    cta: { label: "Open live AI workflow", to: "/matrix/ai-workflow?tenant=mcd" },
  },
  {
    target: "utility",
    ...
    cta: { label: "Open Conflict Hunter", to: "/utility/conflict-hunter?tenant=mcd" },
  },
  {
    target: "portfolio",
    ...
    cta: { label: "Open executive portfolio", to: "/portfolio/executive?tenant=mcd" },
  },
  {
    target: "roi",
    ...
    <div className="space-y-14 pb-20">
        ...
        <div className="relative z-10 grid gap-10 px-6 py-12 md:px-12 md:py-16 lg:grid-cols-[1.35fr_1fr] lg:items-center">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 text-base leading-7 text-muted-foreground md:text-lg">
              ...
              comments, harvesting portals, and pre-clearing drawings the moment a set uploads.
              What the field team feels: openings unlocked weeks earlier, per store, at scale.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/portfolio/executive?tenant=mcd" className="pilot-button-primary">
                Enter live portfolio <ArrowRight className="h-4 w-4" />
              </Link>
              <Link to="/matrix/ai-workflow" className="pilot-button-ghost">
          ...
          {painPoints.map((p) => (
            <div key={p.title} className="pilot-card p-4">
              <div className="flex items-center gap-2">
                ...
                <div className="font-tight text-sm font-bold uppercase tracking-wide">
                  {p.title}
                </div>
              </div>
      ...
      <section data-tour="timeline" className="pilot-card overflow-hidden">
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          <div className="grid gap-3 border-t border-border pt-5 md:grid-cols-3">
            <Delta icon={Clock}       kicker="Elapsed"           value={`${legacyTotal - pilotTotal} weeks saved`} sub={`${legacyTotal} wk → ${pilotTotal} wk end-to-end`} />
            <Delta icon={ClipboardCheck} kicker="Rework loops"   value="1 → 0.2 avg"        sub="DesignCheck resolves comments pre-file" />
            <Delta icon={CheckCircle2} kicker="First-pass approval" value="34% → 84%"       sub="Across 12-store East Coast pilot" />
          </div>
        </div>
      </section>
      ...
      <section data-tour="agents">
        <div className="mb-6 flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          ...
          <div className="text-sm text-muted-foreground">
            Detect change → reason about impact → take the next action, without waiting on a human hop.
          </div>
        </div>
      ...
      <section data-tour="utility" className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        <div className="pilot-card overflow-hidden">
          <header className="flex items-center justify-between border-b border-border bg-muted/30 px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-tight text-base font-bold">MCD-231 · Leesburg VA · utility coordination</h3>
            </div>
            <span className="pilot-kicker text-success">All utilities cleared · 11 calendar days</span>
          </header>
          ...
          <div className="border-t border-border bg-muted/20 px-5 py-3 text-xs leading-5 text-muted-foreground">
            Legacy sequential path for the same store: <span className="font-data text-foreground">34 days</span>{" "}
            across five utilities. PermitPilot runs them in parallel with a shared ticket state so
            civil, gas, and electric never block each other.
          </div>
        </div>
        ...
        <div className="pilot-card p-5">
          ...
          <Link to="/utility/conflict-hunter?tenant=mcd" className="pilot-button-ghost mt-6">
      ...
      <section data-tour="portfolio" className="pilot-card overflow-hidden">
        <header className="flex flex-col gap-2 border-b border-border bg-muted/30 px-6 py-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-warning" /> Watch</span>
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-destructive" /> Escalate</span>
          </div>
        </header>
              ...
              {projects.map((p) => (
                <tr key={p.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-6 py-3 text-right">
                    <span className={cn("ml-auto inline-block h-2.5 w-2.5 rounded-full", riskDot(p.risk))} />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <ol className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {rollout.map((r, i) => (
            <li key={r.title} className="pilot-card-raised relative overflow-hidden p-5">
              <span className="font-display text-6xl font-bold text-primary/10 absolute -right-2 -top-4 select-none">
                {i + 1}
              </span>
      ...
      {/* CTA */}
      <section data-tour="cta" className="pilot-card relative overflow-hidden bg-gradient-to-br from-primary/10 via-background to-pilot-teal/10 p-8 md:p-12">
        <div className="relative grid gap-6 md:grid-cols-[1.4fr_1fr] md:items-center">
          ...
          <div className="flex flex-col gap-3">
            <Link to="/onboarding/authorization?tenant=mcd" className="pilot-button-primary justify-center">
