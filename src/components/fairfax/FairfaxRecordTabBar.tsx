...
/**
 * TEMP: Fairfax portal-data UI — only Record Details + Attachments.
 * Set `false` to restore full Record Info / Payments / Plan Review chrome.
 */
export const FAIRFAX_MINIMAL_SECTIONS_UI = true;
...
interface FairfaxRecordTabBarProps {
  activePanel: DetailPanel;
  onPanelChange: (panel: DetailPanel) => void;
  /** When true (Fairfax temp mode), only Record Details + Attachments. */
...
export function FairfaxRecordTabBar({
  activePanel,
  onPanelChange,
  minimalSections = FAIRFAX_MINIMAL_SECTIONS_UI,
}: FairfaxRecordTabBarProps) {
  if (minimalSections) {
    return (
