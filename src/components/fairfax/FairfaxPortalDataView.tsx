...
import {
  FairfaxRecordTabBar,
  FAIRFAX_MINIMAL_SECTIONS_UI,
  type DetailPanel,
} from "@/components/fairfax/FairfaxRecordTabBar";
...
interface FairfaxPortalDataViewProps {
...
export function FairfaxPortalDataView({
  portalData,
  projectId = null,
  permitNumber = null,
  credentialLoginUrl = null,
}: FairfaxPortalDataViewProps) {
  const [activePanel, setActivePanel] = useState<DetailPanel>("record_details");

  const showFullSections = !FAIRFAX_MINIMAL_SECTIONS_UI;
  ...
  useEffect(() => {
    if (!FAIRFAX_MINIMAL_SECTIONS_UI) return;
  ...
  useEffect(() => {
    ...
    console.log("[FairfaxPortalDataView] binding", {
      projectId,
      permitNumber,
      credentialLoginUrl: credentialLoginUrl ? "(set)" : null,
      fairfaxMode: isEmbeddedInPortalData ? "embedded" : "standalone",
      portalDataKeys: {
  ...
  return (
    <div className="min-h-full space-y-6">
      <div className="rounded-md border border-border/80 bg-muted/30 px-3 py-2">
        <p className="text-xs font-medium text-muted-foreground">Fairfax County, VA</p>
        <p className="text-sm font-semibold text-foreground">Citizen Access (Accela)</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <FairfaxRecordTabBar
            ...
            minimalSections={FAIRFAX_MINIMAL_SECTIONS_UI}
          />
